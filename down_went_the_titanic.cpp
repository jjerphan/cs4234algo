#include <bits/stdc++.h>

using namespace std;

const int INF = 2000000000;

struct Edge {
	int from, to, cap, flow, index;
	Edge(int from, int to, int cap, int flow, int index) :
		from(from), to(to), cap(cap), flow(flow), index(index) {}
};

struct Dinic {
	int N;
	vector<vector<Edge> > G;
	vector<Edge *> dad;
	vector<int> Q;

	Dinic(int N) : N(N), G(N), dad(N), Q(N) {}

	void AddEdge(int from, int to, int cap) {
		G[from].push_back(Edge(from, to, cap, 0, G[to].size()));
		if (from == to) G[from].back().index++;
		G[to].push_back(Edge(to, from, 0, 0, G[from].size() - 1));
	}

	long long BlockingFlow(int s, int t) {
		fill(dad.begin(), dad.end(), (Edge *) NULL);
		dad[s] = &G[0][0] - 1;

		int head = 0, tail = 0;
		Q[tail++] = s;
		while (head < tail) {
			int x = Q[head++];
			for (int i = 0; i < G[x].size(); i++) {
	Edge &e = G[x][i];
	if (!dad[e.to] && e.cap - e.flow > 0) {
		dad[e.to] = &G[x][i];
		Q[tail++] = e.to;
	}
			}
		}
		if (!dad[t]) return 0;

		long long totflow = 0;
		for (int i = 0; i < G[t].size(); i++) {
			Edge *start = &G[G[t][i].to][G[t][i].index];
			int amt = INF;
			for (Edge *e = start; amt && e != dad[s]; e = dad[e->from]) {
	if (!e) { amt = 0; break; }
	amt = min(amt, e->cap - e->flow);
			}
			if (amt == 0) continue;
			for (Edge *e = start; amt && e != dad[s]; e = dad[e->from]) {
	e->flow += amt;
	G[e->to][e->index].flow -= amt;
			}
			totflow += amt;
		}
		return totflow;
	}

	long long GetMaxFlow(int s, int t) {
		long long totflow = 0;
		while (long long flow = BlockingFlow(s, t))
			totflow += flow;
		return totflow;
	}
};


int N, X, Y, P;

int id(int x, int y, bool in) {
    return x + y * X + (in ? 0 : X * Y);
}

int in(int x, int y) { return id(x, y, true); }
int out(int x, int y) { return id(x, y, false); }

int main() {
    while (cin >> X) {
        cin >> Y;
        cin >> P;
        N = 2 * X * Y + 2;
        int source = 2 * (X * Y);
        int target = source + 1;
        Dinic solver(N);
        char parsed;

        for (int i = 0 ; i < X ; i++) {
            for (int j = 0 ; j < Y ; j++) {
                cin >> parsed;

                // In and out vertex for a given position
                int inn = in(i, j);
                int outt = out(i, j);

                // Connecting neighbours
                if (i > 0) {
                    solver.AddEdge(outt, in(i - 1, j), INF);
                }

                if (i < X - 1) {
                    solver.AddEdge(outt, in(i + 1, j), INF);
                }

                if (j > 0) {
                    solver.AddEdge(outt, in(i, j - 1), INF);
                }


                if (i < Y - 1) {
                    solver.AddEdge(outt, in(i, j + 1), INF);
                }

                if (parsed == '*') {  // People floating on ice
                    // People on the ice
                    solver.AddEdge(source, inn, 1);
                    // Vertex cap
                    solver.AddEdge(inn, outt, 1);
                }

                if (parsed == '@') {  // Iceberg
                    // Vertex cap
                    solver.AddEdge(inn, outt, 2);
                }

                if (parsed == '.') {  // Floating ice
                    // Vertex cap
                    solver.AddEdge(inn, outt, 1);
                }

                if (parsed == '#') {  // Wood
                    // Vertex cap
                    solver.AddEdge(inn, outt, P);
                    // "Meeting point"
                    solver.AddEdge(outt, target, INF);
                }
            }
        }

        cout << solver.GetMaxFlow(source, target) << "\n";

    }
    return 0;
}
