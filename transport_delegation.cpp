...
int main() {
    ...
    // cout << "Raw sites \n";
    for (int i = 0; i < r ; i++) {
        site_int = get_next_site_id();
        solver.add_edge(source, site_int, 1);
    }
    ...
    for (int i = 0; i < f ; i++) {
        site_int = get_next_site_id();
        solver.add_edge(site_int, target, 1);
    }

    // Tranport company : vertex capacity and connection to sites
    int nb_state_accessible = 0;
    for (int i = 0 ; i < t ; i++) {
        // cout << "Transport company " << i << " \n";

        int in = off_set_trans + i;
        int out = in + t;

        // Vertex capacity
        solver.add_edge(in, out, 1);
        cin >> nb_state_accessible;
        for (int j = 0 ; j < nb_state_accessible; j++) {
            site_int = get_next_site_id();
            // Linking both ways
            solver.add_edge(site_int, in, 1);
            solver.add_edge(out, site_int, 1);
        }
    }
