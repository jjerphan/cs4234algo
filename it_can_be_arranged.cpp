...
void resolve_case() {
    ...
    int beg, end, students, rooms;
    vector<course > courses(N);

    MaxFlow solver(2 + 2 * N);
    int source = 0;
    int target = 1 + 2* N;
    ...
    for (int n = 0 ; n < N ; n++) {
        ...
        courses[n].rooms = ceil(static_cast<float> (students) / M);
        N_rooms += courses[n].rooms;
        solver.add_edge(source, n + 1, courses[n].rooms);
        solver.add_edge(n + 1 + N, target, courses[n].rooms);
    }

    // Reading cleaning time and infering if it's possible to use
    // some rooms
    int cleaning_time = 0;
    for (int i = 0; i < N ; i++) {
        for (int j = 0 ; j < N ; j++) {
            cin >> cleaning_time;
            if (i != j) {
                if (courses[i].end + cleaning_time < courses[j].beg) {
                    solver.add_edge(1 + i, 1 + j + N, 1000000);
                }
            }
        }
    }

    cout << N_rooms - solver.push_relabel() << "\n";
