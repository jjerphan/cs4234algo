...
class MaxFlow {
 ...
 public:
    ...
    int push_relabel(int _s, int _t) {
...
int main() {
    ...
    while (1) {
        ...
        MaxFlow solver(2 + n_robots + n_holes);
        ...
        for (int i = 0; i < n_holes; i++) {
            solver.add_edge(i + n_robots, target, 1);
        }
        ...
        for (int t = 0; t < 3; t++) {
            ...
            while (index_second < seconds_needed.size()) {
                ...
                solver.add_edge(r, h, 1);
                index_second++;
            }

            int res = solver.push_relabel(source, target);
